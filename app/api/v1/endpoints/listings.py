...
def get_listing_filters(db: Session = Depends(get_db)):
    ...
    return {"neighborhoods": [n[0] for n in neighborhoods], "types": [t[0] for t in types]}

@router.get("/properties/{id}", response_model=schemas.Listing)
def get_property(id: int, db: Session = Depends(get_db)):
    prop = db.query(models.Listing).filter(models.Listing.id == id).first()
    if not prop:
...
def create_property(
    listing: schemas.ListingCreate, db: Session = Depends(get_db),
    ...
    return crud.create_listing(db=db, listing=listing, user_id=db_user.id if db_user else None)

@router.put("/properties/{id}", response_model=schemas.Listing)
def update_property(
    id: int, listing: schemas.ListingUpdate, db: Session = Depends(get_db),
    current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))
):
    db_user = db.query(models.User).filter(models.User.email == current_user["email"]).first()
    updated = crud.update_listing(db=db, listing_id=id, listing_update=listing, user_id=db_user.id if db_user else None)
    if not updated:
        raise HTTPException(status_code=404, detail="Property not found")
    return updated
...
def delete_property(id: int, db: Session = Depends(get_db),
    ...
    if not crud.delete_listing(db=db, listing_id=id):
        raise HTTPException(status_code=404, detail="Property not found")

# --- RESEARCH LISTINGS ---

@router.get("/admin/research-listings", response_model=List[schemas.ResearchListing])
def get_research_listings(skip: int = 0, limit: int = 100, db: Session = Depends(get_db),
                           current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))):
...
def create_research_listing(listing: schemas.ResearchListingCreate, db: Session = Depends(get_db),
                             current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))):
    try:
...
async def send_research_mail(payload: schemas.SendResearchMailRequest, db: Session = Depends(get_db),
                              current_user: dict = Depends(auth.RoleChecker(["superuser"]))):
    ...
    investors = (db.query(models.Investor).filter(models.Investor.id.in_(payload.investor_ids)).all()
                 if payload.investor_ids else db.query(models.Investor).all())
    if not investors:
        raise HTTPException(status_code=400, detail="No matching investors found")
    count = await mail_service.send_research_listing_to_investors(
        investors=investors, listing=listing,
        image_url=payload.image_url, additional_message=payload.additional_message)
    return {"message": f"Successfully sent mail to {count} investors"}

@router.put("/admin/research-listings/{id}", response_model=schemas.ResearchListing)
def update_research_listing(id: int, updates: schemas.ResearchListingUpdate, db: Session = Depends(get_db),
                             current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="This URL is already assigned to another listing.")

@router.delete("/admin/research-listings/{id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_research_listing(id: int, db: Session = Depends(get_db),
                             current_user: dict = Depends(auth.RoleChecker(["superuser"]))):
    if not crud.delete_research_listing(db=db, listing_id=id):
        raise HTTPException(status_code=404, detail="Research listing not found")

# --- RESEARCH TAGS ---

@router.get("/admin/research-tags", response_model=List[schemas.ResearchTag])
def get_research_tags(db: Session = Depends(get_db),
                      current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))):
    return crud.get_research_tags(db)

@router.post("/admin/research-tags", response_model=schemas.ResearchTag)
def create_research_tag(tag: schemas.ResearchTagCreate, db: Session = Depends(get_db),
                        current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))):
    try:
...
def delete_research_tag(id: int, db: Session = Depends(get_db),
                        current_user: dict = Depends(auth.RoleChecker(["superuser", "editor"]))):
    if not crud.delete_research_tag(db, id):
